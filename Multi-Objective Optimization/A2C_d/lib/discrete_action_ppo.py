...
if __name__ == "__main__":
    ...
    test_env = environ_discreate_action.SocialEnv()
    ...
    net_act = agent_model.ModelActor(envs[0].observation_space.shape, envs[0].action_space.n, hidden=32).to(device)
    net_crt = agent_model.ModelCritic(envs[0].observation_space.shape, hidden=32).to(device)
    print(net_act)
    print(net_crt)

    # agent = model.AgentA2C(net_act, device=device)
    agent = agent_model.AgentPPO(net_act, apply_softmax=True, device=device)
