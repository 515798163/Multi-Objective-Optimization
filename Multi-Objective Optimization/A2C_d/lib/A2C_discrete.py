...
import environ_discreate_action
import agent_model
...
def unpack_batch(batch, net, device='cpu'):
    ...
    for idx, exp in enumerate(batch):
        ...
        actions.append(exp.action.astype(int))
...
if __name__ == "__main__":
    ...
    for i in range(NUM_ENVS):
        envs.append(environ_discreate_action.SocialEnv())

    # make_env = lambda: ptan.common.wrappers.wrap_dqn(env)
    # envs = [make_env() for _ in range(NUM_ENVS)]

    writer = SummaryWriter(comment='-SocialNetwork')

    net = SocialA2C(envs[0].observation_space.shape, envs[0].action_space.n, hidden=32).to(device)
    # '''加载模型的方式'''
    # net = torch.load('../model/2022-04-15-16-33-01.pth')
    print(net)

    agent = agent_model.AgentPPO(lambda x: net(x)[0], apply_softmax=True, device=device)
    ...
    with common_class.RewardTracker(writer, stop_reward=3.9) as tracker:
        with ptan.common.utils.TBMeanTracker(writer, batch_size=10) as tb_tracker:
            for step_idx, exp in enumerate(exp_source):
                ...
                log_prob_actions_v = torch.zeros((BATCH_SIZE, BATCH_SIZE)).to(device)
                for i in range(CRUMOR_NUMBER):
                    log_prob_actions_v += adv_v * log_prob_v[range(BATCH_SIZE), actions_t[:, i]]
