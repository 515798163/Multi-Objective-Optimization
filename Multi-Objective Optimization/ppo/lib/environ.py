...
class SocialEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # action为多维动作，如[0.4011, 0.3826, -0.2315, 0.0657, 0.0002]
        # 需要把[-1, 1]的动作，映射到[0, 4038]上
        # 返回的action是一个numpy数组，元素是list，如[[1.5 2.  3. ]]
        action = (action + 1) / 2
        for i in range(len(action)):
            example = round(action[i])
            action[i] = example
        # 在动作空间中选择节点
        action_list = self.action.select_node(action)
