...
NORMALIZE_all_FEATURE_PATH = '../data/wiki_vote/wiki_vote_nprmalize_features.csv'
...
logging.basicConfig(level=logging.INFO,
                    filename='../log/all_reward_t=6.txt',
                    filemode='w',
...
class Actions:
    def __init__(self):
        self.n = NODE_NUMBER - SociaNetwork.RUMOR_NUMBER
        self.action_space = {}
        self.update_action_space()

    '''根据社交网络图，更新动作空间'''
    def update_action_space(self):
        all_actions = range(NODE_NUMBER)
        actions_list = list(set(all_actions) - set(SociaNetwork.R_SOURCES))
        for i in range(self.n):
            self.action_space[i] = actions_list[i]
...
class State:
    ...
    def reset(self):
        '''初始化特征作为状态'''
        features_pd = pd.read_csv(NORMALIZE_all_FEATURE_PATH)
        self.features = normalize(np.asarray(features_pd, dtype=np.float32))
...
class SocialEnv(gym.Env):
    ...
    def step(self, action_idx):
        ...
        if is_done:
            ...
            logging.info("reward:{},c_sources:{}".format(reward, self.graph.c_sources))
            # 当t=0时-0.5 当t=1时-1.5   当t=2时-2.5   当t=3时-3.8   当t=4时-4.8   当t=5时-5.8   当t=6时-6.0
            if reward >= -6.0:
                ...
                for i in infect_time:
                    if i > max_time and i != SociaNetwork.MAX_TIME:
                        max_time = i
                logging.info("reward:{},c_sources:{},infect_time:{}".format(reward, self.graph.c_sources, max_time))
